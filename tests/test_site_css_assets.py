...
class SiteCssAssetsTests(unittest.TestCase):
    ...
    def test_base_css_defines_shared_workshop_theme_tokens(self) -> None:
        self.assertIn("color-scheme: light;", self.base_css)
        self.assertIn("--color-bg: #f3f6fb;", self.base_css)
        self.assertIn("--color-text: #1e3047;", self.base_css)
        self.assertIn("--color-accent: #345ee8;", self.base_css)
    ...
    def test_base_css_uses_plain_reading_background(self) -> None:
        self.assertIn("background-color: var(--color-bg);", self.base_css)
        self.assertNotIn("background-image: radial-gradient(", self.base_css)
    ...
    def test_layout_css_covers_catalog_shell_and_responsive_sidebar(self) -> None:
        ...
        self.assertIn("grid-template-columns: 15rem minmax(0, 1fr);", self.layout_css)
